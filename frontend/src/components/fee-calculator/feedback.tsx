...
export function FeeCalculatorFeedback(props: FeeCalculatorFeedbackProps) {
  ...
  return (
    <>
      ...
      {result ? (
        <div className="overflow-hidden rounded-2xl border border-emerald-100 bg-emerald-50 px-6 py-5 shadow-sm">
          <div className="flex items-center gap-3 border-b border-emerald-100 pb-4">
            ...
            <h3 className="m-0 text-xl font-bold tracking-tight text-emerald-950">
              {LABEL.completeTitle}
            </h3>
          </div>
          <div className="mt-4 space-y-3 pt-2">
            {toSummaryItems(result).map((item) => (
              <div key={item.label} className="flex items-center justify-between text-base">
                ...
                <span className="font-bold text-emerald-950">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
